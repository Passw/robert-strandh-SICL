...
(defun current-package (client)
  (let* ((global-environment (environment client))
    ...
    (cbe:symbol-value '*package* package-cell)))
...
(defun define-package-functions (client global-environment)
  ...
  (setf (clo:fdefinition client global-environment 'cl:symbol-package)
        (fdefinition (find-symbol (symbol-name 'cl:symbol-package)
                                  '#:sicl-new-boot-parcl-extrinsic)))
  (setf (clo:fdefinition client global-environment 'package-name)
        (fdefinition (find-symbol (symbol-name 'package-name)
                                  '#:sicl-new-boot-parcl-extrinsic)))
  ...
  (setf (clo:fdefinition client global-environment 'export)
        (fdefinition (find-symbol (symbol-name 'export)
                                  '#:sicl-new-boot-parcl-extrinsic)))
  (setf (clo:fdefinition client global-environment 'import)
        (fdefinition (find-symbol (symbol-name 'import)
                                  '#:sicl-new-boot-parcl-extrinsic)))
